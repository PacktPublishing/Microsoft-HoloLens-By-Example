...
public class PWNetworkManager : NetworkManager {    
    ...
    public bool IsReady
    {
        get
        {
            return _isReady || AppManager.Instance.mode == AppManager.NetworkingMode.None; 
        }
        private set
        {
            _isReady = value;
    ...
    private void Start()
    {
        ...
        switch (AppManager.Instance.mode)
        {
            case AppManager.NetworkingMode.LocalDiscovery:
                GetComponent<PWNetworkDiscovery>().StartScanning();
                break;
            case AppManager.NetworkingMode.Server:
                StartServer();
                break;
            case AppManager.NetworkingMode.Client:
                StartClient();
                break; 
        }        
    }
